    ...
    pub(super) fn get_value<F: FnMut() -> WResult<usize>>(
        ...
        let r = if let Some(ty) = ty {
            match (ty.as_ref(), val) {
                ...
                (TypeBlockData::MetaData, _) => GlobalValueUnit::Meta(idx),
...
impl<F: Read + Seek> BitReader<F> {
    ...
    pub(in crate::bitcode) fn parse_function_block(
        ...
        let mut metadata_attachment = None;
                ...
                BitCodeEntry::SubBlock(id, state_sub) => {
                    ...
                    match id {
                        ...
                        BlockIDs::MetaDataAttachment => {
                            let t = self.parse_metadata_attachment_block(
                                state_sub,
                                block_infos,
                                metadata,
                                metadata_kind,
                            )?;
                            metadata_attachment.replace(t);
                        }
        ...
        Ok(FunctionBlock {
            ...
            vars: value_list,
            metadata_attachment: metadata_attachment.unwrap_or_default(),
            idxs_instr,
            idxs_instr_global,
            idx_start,
        })
