...
impl FunctionInstrTrait for InstrInsertElt {
    ...
    fn new<'a>(mut data: FunctionInstrData<'a>) -> WResult<Self> {
        let src = data.next_value("parse insert_elt src", None)?;

        let ty = src.get_ty("parse insert_elt src ty")?;
        ...
        let elt = data.next_value("parse insert_elt elt", Some(ty_unit))?;

        let idx = data.next_value("parse insert_elt idx", None)?;
