...
use crate::bitcode::blocks::function_block::utils::{
    FunctionInstrData, FunctionInstrTrait, parse_alignment,
};
...
impl FunctionInstrTrait for InstrAlloca {
    ...
    fn new<'a>(mut data: FunctionInstrData<'a>) -> WResult<Self> {
        if data.data.len() < 4 {
            return Err(WError::DataNotFound("parse alloca"));
        }
        let ty_inst = data.next_ty("alloca inst ty")?;
        let ty_op = data.next_ty("alloca op ty")?;
        let idx_size = data.data.next("parse alloca size idx")?;
        let size = data
            .other
            .values
            .get(idx_size)
            ...
            .and_then(|v| {
                if let GlobalValueUnit::Data(v) = v {
                    Some(v.value_inner().try_into())
                } else {
                    None
                }
            })
            .ok_or(WError::DataNotFound("alloca size"))??;
        let idx_align_flags = data.data.next("parse alloca align flags")?;
        let align = {
        ...
        let address_space = if let Ok(v) = data.data.next("parse alloca inst ty idx") {
            v as u16
        } else {
