...
use crate::bitreader::decoded_signed_vbrs;
...
impl FunctionInstrTrait for InstrPhi {
    ...
    fn new<'a>(mut data: FunctionInstrData<'a>) -> WResult<Self> {
        ...
        let srcs = (0..num)
            .map(|_| {
                let idx_val = data.data.next("parse phi val idx")? as i64;
                let idx_val = decoded_signed_vbrs(idx_val) as usize;
                let (val, idx_val) =
                    data.other
                        .get_value(idx_val, Some(ty.clone()), "parsh phi val", || {
                            Err(WError::DataNotFound("parse phi val ty"))
                        })?;
