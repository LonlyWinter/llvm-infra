...
enum_parse! {
    ...
    Fdiv = (InstrDataFastMathFlags; InstrDataFastMathFlags::default(); (4, true)),
    Frem = (InstrDataFastMathFlags; InstrDataFastMathFlags::default(); (6, true)),
}
...
impl FunctionInstrTrait for InstrBinOP {
    ...
    fn new<'a>(mut data: FunctionInstrData<'a>) -> WResult<Self> {
        let left = data.next_value("parse binop left", None)?;

        let ty = left.get_ty("binop left ty")?;
        let fp = ty.is_float() || ty.is_float_vec();

        let right = data.next_value("parse binop right", Some(ty.clone()))?;

        // eprintln!("binop: {:?} {:?}", left, right);

        let mut op = {
            let op = data.data.next("parse binop op")?;
            InstrBinOPType::try_from((op, fp))?
        };

        if let Ok(info) = data.data.next("parse binop info") {
            match &mut op {
