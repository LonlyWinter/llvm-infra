    ...
    ("mangling", $($name:ident = $value:expr)*) => {
        ...
        impl DataLayoutMangling {
            fn parse(chars: Vec<char>) -> WResult<Self> {
                if chars.len() != 2 {
                    return Err(WError::DataNotAllowed("parse mangling", String::from_iter(chars)));
                }
...
fn parse_integer<T: FromStr>(chars: Vec<char>) -> WResult<T> {
    ...
    t.parse::<T>()
        .ok()
        .ok_or(WError::DataNotAllowed("parse integer", t))
}
...
fn parse_primitive(
    ...
    let bit_width = {
        let bit_width = chars
            .next()
            .ok_or(WError::DataNotFound("parse primitives bit_width"))?;
    ...
    let align_abi = {
        let align_abi = chars
            .next()
            .ok_or(WError::DataNotFound("parse primitives align"))?;
...
fn parse_pointer(chars: Vec<char>) -> WResult<DataLayoutPointer> {
    ...
    let mut chars_address_space = chars
        .next()
        .ok_or(WError::DataNotFound("parse pointer address_space"))?;
    ...
    while let Some(front) = chars_address_space.pop() {
        if front == 'e' {
            ...
            chars_address_space.reverse();
            return Err(WError::DataNotAllowed(
                "parse pointer address_space error flag",
    ...
    if address_space == 0 && (external_state || unstable_repr) {
        return Err(WError::DataNotAllowed(
            "parse pointer address_space error state",
            format!(
...
fn parse_ni(mut chars: Vec<char>) -> WResult<Vec<u16>> {
    if chars.len() < 2 {
        return Err(WError::DataNotFound("parse ni"));
    }
    let f = chars.remove(0);
    if f != ':' {
        return Err(WError::DataNotAllowed("parse ni", String::from_iter(chars)));
    }
            ...
            match data_front {
                ...
                'F' => {
                    if data_now_rest.len() < 2 {
                        return Err(WError::DataNotFound("parse function ptr align"));
                    }
