...
use crate::{
    ...
    bitcode::{
        blocks::{
            ...
            typeblock::{TypeBlock, TypeBlockData, TypeBlockUnit},
        },
        entry::BitCodeEntryRecord,
        parse::StateBlock,
        parse_data_to_string,
    },
    filebit::{
        BitReader, decoded_signed_vbrs_u8, decoded_signed_vbrs_u32, decoded_signed_vbrs_u64,
    },
};
...
macro_rules! enum_constants {
    ...
    ("method_integer"; $($name_integer:ident, $name_wide:ident, $func_vbr:ident, $ty_vbr:ty, $ty_unsign:ty;)*) => {
        $(
            fn $name_integer(data: Vec<u64>) -> WResult<$ty_unsign> {
                ...
                let raw = data[0] as $ty_vbr;
                let res = $func_vbr(raw) as $ty_unsign;
                Ok(res)
            }

            fn $name_wide(data: Vec<u64>) -> WResult<Vec<$ty_unsign>> {
                let res = data.into_iter()
                    .map(| v | v as $ty_unsign)
                    .collect::<Vec<_>>();
                Ok(res)
            }
        )*
    ...
    ("value_single_string"; $($name:ident,)*) => {
        impl TryInto<String> for ValueSingle {
            ...
            fn try_into(self) -> WResult<String> {
                match self {
                    $(
                        Self::$name(v) => Ok(parse_data_to_string(v.into_iter())),
                    )*
...
enum_constants! {
    "method_integer";
    parse_constants_integer8, parse_constants_vec_integer8, decoded_signed_vbrs_u8, u8, u8;
    parse_constants_integer16, parse_constants_vec_integer16, decoded_signed_vbrs_u32, u32, u16;
    parse_constants_integer32, parse_constants_vec_integer32, decoded_signed_vbrs_u32, u32, u32;
    parse_constants_integer64, parse_constants_vec_integer64, decoded_signed_vbrs_u64, u64, u64;
    parse_constants_f32, parse_constants_vec_f32, decoded_signed_vbrs_u32, u32, f32;
    parse_constants_f64, parse_constants_vec_f64, decoded_signed_vbrs_u64, u64, f64;
}

// enum_constants! {
//     "method_as";
//     parse_constants_integer64, parse_constants_vec_integer64, u64;
...
fn parse_constants_integer128(data: Vec<u64>) -> WResult<u128> {
    let data = data
        .into_iter()
        .map(decoded_signed_vbrs_u64)
        .collect::<Vec<_>>();
    let res = parse_constants_bit128(data)?;
    Ok(res)
}
...
impl<F: Read + Seek> BitReader<F> {
    pub(in crate::bitcode) fn parse_constants_block(
        ...
        let mut parse_constants_block_now = |data: BitCodeEntryRecord<u64>| {
            ...
            if !(data.blob.is_empty() && data.chars.is_empty()) {
                return Err(WError::DataNotAllowed(
                    "parse constant blob/char",
                    format!("blob: {:?}, char: {}", data.blob, data.chars),
                ));
            }
