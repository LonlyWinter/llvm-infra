...
macro_rules! enum_metadata {
    (
        $($key:ident = $value1:pat, $value2:pat)*
    ) => {
...
fn parse_meta_data_kind_block(code: u32, data: Vec<u16>) -> WResult<(usize, String)> {
...
pub(in crate::bitcode) type MetaDataKindBlock = Vec<(usize, MetaDataKindMap)>;
...
impl<F: Read + Seek> BitReader<F> {
    pub(in crate::bitcode) fn parse_meta_data_kind_block(
        ...
        let mut parse_meta_data_kind_block_now = |data: BitCodeEntryRecord<u16>| {
            ...
            let v = MetaDataKindMap::try_from(v)?;
            res.push((k, v));
            Ok(())
        };
