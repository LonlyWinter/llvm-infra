...
impl GlobalValueUnit {
    pub(super) fn get_ty(&self, tag: &'static str) -> WResult<TypeBlockUnit> {
        let r = match &self {
            ...
            GlobalValueUnit::GlobalVar(_v) => todo!(),
            _ => {
...
pub(in crate::bitcode) struct ModuleBlock {
    ...
    pub(in crate::bitcode) metadata: MetaData,
    pub(in crate::bitcode) named_metadata: NamedMetaData,
...
impl<F: Read + Seek> BitReader<F> {
    pub(in crate::bitcode) fn parse_module_block(
        ...
        let mut metadata_kind = Vec::with_capacity(64);
        let mut metadata = MetaData::default();
        let mut named_metadata = HashMap::with_capacity(64);
                ...
                BitCodeEntry::Record(data) => {
                    ...
                    match content {
                        ...
                        ModuleBlockContents::SourceFileName => {
                            basic.source_filename = data.chars;
                        }
