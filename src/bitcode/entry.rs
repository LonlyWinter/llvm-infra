...
use crate::{WError, WResult, bitcode::blocks::blockinfo::BlockInfoAll};
...
const MAX_NUM_ELEMENTS: usize = 4096;
...
impl<T: BitType> DataNext<T> {
    ...
    pub(super) fn collect(self) -> Vec<T> {
        self.data.collect()
    }
...
impl<F: Read + Seek> BitReader<F> {
    ...
    pub(in crate::bitcode) fn parse_entry<T: BitType>(
        ...
        while self.has_data() {
            ...
            // eprintln!("entry parse abbrev: {:?}, size: {}, idx: {}", code, code_size, idx);
            match code {
        ...
        Err(WError::DataNotFound("Parse entry"))
    }
